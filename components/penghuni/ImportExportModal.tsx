...
import { parsePenghuniPaste, HasilParsePenghuni, PenghuniPasteInput } from "@/lib/paste-penghuni";
import { generatePenghuniCSV, triggerFileDownload, downloadTemplatePenghuniCSV, TenantExportData } from "@/lib/excel-penghuni";
...
// helper --------------------------------------------------------------------------
// function Modal Komponen Import & Export Data Penghuni
...
export default function ImportExportModal({ isOpen, onClose, tenants, onSuccess }: ImportExportModalProps) {
  ...
  // helper --------------------------------------------------------------------------
  // function untuk membaca file CSV dari input file
  ...
  // helper --------------------------------------------------------------------------
  // function untuk mengunduh data penghuni saat ini ke file CSV
  // input param : none
  // output : void
  // end of helper ------------------------------------------------------------------
  const handleExportCSV = () => {
    const csvData = generatePenghuniCSV(tenants);
    const dateStr = new Date().toISOString().split("T")[0];
    triggerFileDownload(csvData, `Data_Penghuni_ABI_Homestay_${dateStr}.csv`);
  };
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-end md:items-center justify-center p-0 md:p-4">
      ...
      {/* Modal Card */}
      <div className="relative w-full md:w-[650px] bg-surface rounded-t-3xl md:rounded-3xl shadow-2xl max-h-[90vh] flex flex-col overflow-hidden pb-safe z-10 animate-slide-up">
        {/* Top Indicator */}
        <div
        ...
        <div className="p-6 overflow-y-auto hide-scrollbar flex-1 space-y-4">
          ...
          {activeTab === "import" ? (
            <div className="space-y-4">
              ...
              <div className="p-4 bg-surface-container-low rounded-2xl border border-surface-variant text-body-sm text-on-surface-variant space-y-2">
                <div className="flex items-start gap-2 text-primary font-bold">
                  ...
                  <span>Petunjuk Format Salin/Tempel:</span>
                </div>
                <p className="pl-6 text-xs text-outline leading-relaxed">
                  Salin (Ctrl+C) data dari <strong>Excel</strong> atau <strong>Google Sheets</strong> dengan urutan kolom:
                </p>
                ...
                <div className="pt-2 flex items-center justify-between">
                  ...
                  >
                    ...
                    Unduh Template CSV Kosong
                  </button>
                  <label className="text-secondary font-label-sm text-xs cursor-pointer hover:underline flex items-center gap-1 font-semibold">
              ...
              <div>
                <label className="font-label-sm text-on-surface-variant mb-1.5 block">
                  Tempel Tabel / Teks Spreadsheet Di Sini:
                </label>
                <textarea
                  rows={4}
                  value={rawText}
            ...
            <div className="py-4 space-y-6">
              <div className="p-5 bg-surface-container-low rounded-2xl border border-surface-variant flex items-center justify-between">
                <div>
                  ...
                  <p className="font-body-sm text-xs text-on-surface-variant mt-0.5">
                    Data siap diekspor ke format CSV / Excel
                  </p>
                </div>
              ...
              <div className="p-4 bg-surface-container-low rounded-2xl border border-surface-variant text-xs text-on-surface-variant space-y-2">
                <div className="flex items-center gap-2 text-primary font-bold">
                  <span className="material-symbols-outlined text-brand-teal text-lg">description</span>
                  <span>Informasi Kolom CSV:</span>
                </div>
                <p className="text-outline leading-relaxed">
                  File ekspor mencakup nama penghuni, nomor kamar, nomor WhatsApp, tanggal masuk, tanggal jatuh tempo, tipe sewa, status sewa, serta harga nominal sewa.
                </p>
              </div>
              ...
              <button
                ...
                className="w-full py-4 rounded-xl bg-brand-teal text-white font-label-md text-label-md shadow-md flex items-center justify-center gap-2 hover:bg-brand-deep-blue transition-all active:scale-95"
              >
                <span className="material-symbols-outlined text-xl">download</span>
                Unduh Data Penghuni (.csv)
              </button>
            </div>
          )}
        </div>
